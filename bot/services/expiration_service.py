"""
expiration_service.py - Monitora ofertas publicadas para detectar expiração ou mudança de preço.
"""
import json
import logging
import datetime
...
def _save_db(data: list) -> None:
    try:
        ...
        # Mantém apenas os últimos 500 itens para não crescer infinitamente
...
async def check_expirations(bot: Bot) -> None:
    ...
    # Verifica apenas ofertas não expiradas das últimas 24h
    for item in db:
        if item.get("expired"):
            continue
        ...
        try:
            ...
            if not data:
                is_expired = True
                reason = "Produto não encontrado ou removido."
            elif not data.get("price"):
                is_expired = True
                reason = "Preço não disponível ou indisponível."
            # Opcional: comparar preço? 
            # Se subir mais de 20%, podemos considerar "encerrada" se for promoção relâmpago
            ...
            if is_expired:
                ...
                for msg_info in item["messages"]:
                    try:
                        ...
                        # Nota: Se for foto, o caption é editado. Se for texto, o text.
                        # Buscamos a mensagem original primeiro? Não precisa, edit_message_caption resolve.
                        
                        warning_text = "\n\n❌ <b>PROMOÇÃO ENCERRADA</b> ❌"
                        ...
                        try:
                            ...
                            await bot.edit_message_caption(
                                ...
                                caption=f"⚠️ <b>ESTA OFERTA JÁ EXPIROU</b> ⚠️\n{reason}{warning_text}",
                            ...
                            await bot.edit_message_text(
                                chat_id=msg_info["chat_id"],
                                message_id=msg_info["message_id"],
                                text=f"⚠️ <b>ESTA OFERTA JÁ EXPIROU</b> ⚠️\n{reason}{warning_text}",
                                parse_mode=ParseMode.HTML
                            )
            ...
            # Pequeno delay entre verificações para não ser bloqueado pela loja
