...
async def process_all_forwardings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for index, item in enumerate(fila):
        ...
        # Mantem a copy original e remove TODOS os links encontrados (vao para o botao)
        urls = re.findall(r'https?://[^\s<>"]+', texto_original)
        ...
        # Se removeu links, garante que o usuario saiba que deve clicar no botao
        if link_principal_afiliado and "botao abaixo" not in copy_gerada.lower():
            copy_gerada += "\n\n🔗 <b>Acesse a oferta clicando no botao abaixo! 👇</b>"
