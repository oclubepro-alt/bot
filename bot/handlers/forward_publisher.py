...
async def enviar_com_midia(bot, chat_id, midia, texto, affiliate_url: str | None = None, reply_markup=None):
    ...
    # Se nao veio o teclado pronto, mas tem link, cria o botao de oferta
...
async def start_forward_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "Pode enviar varias — eu organizo tudo pra você.\n\n"
...
async def receive_forwarded_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Verificacao de Marca d'agua (Fase 1)
    if midia.get("tipo") == "photo":
        try:
            ...
            if tem_marca:
                logger.warning(f"[ENCAM] 🚫 Marca d'agua detectada. Ignorando mensagem.")
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text="🚫 <b>Mensagem ignorada!</b>\nDetectamos uma marca d'agua ou logo de outro canal nesta imagem.",
                    parse_mode="HTML"
                )
                return
        except Exception as e:
            logger.error(f"[ENCAM] Erro ao processar visao: {e}")
...
async def finalizar_lote_encaminhamento(context, chat_id, user_id):
    logger.info(f"[TIMER] ⏳ Iniciando finalizacao de lote para {user_id}")
    
    # Tenta pegar o user_data do contexto ou da aplicacao (fallback)
    ...
    # Tenta apagar a mensagem de status anterior para nao poluir
    if msg_id:
        try:
    ...
    try:
        # Sempre envia uma NOVA mensagem no final do lote para ficar no rodape do chat
...
async def encam_add_cupom(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "🎫 <b>MODO CUPOM (BATCH)</b>\n\nEnvie o codigo do cupom que sera aplicado à <b>ultima mensagem</b> recebida:",
        parse_mode="HTML",
...
async def process_all_forwardings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Salva o total ANTES de processar para o contador da revisao nao quebrar
    ...
    for index, item in enumerate(fila):
        ...
        # Mantem a copy original mas remove todos os links para evitar cliques no link do autor original
        # 1. Remove tags <a> mantendo apenas o texto (ex: <a href="...">Texto</a> -> Texto)
        copy_gerada = re.sub(r'<a\s+href=["\'][^"\']+["\'][^>]*>(.*?)</a>', r'\1', texto_original, flags=re.IGNORECASE | re.DOTALL)
        
        # 2. Identifica todas as URLs puras no texto original para processamento
        urls_originais = re.findall(r'https?://[^\s<>"]+', texto_original)
        link_principal_afiliado = None

        for url_puro in urls_originais:
            # Tenta injetar o link de afiliado (para usar no botao)
            link_af = await injetar_link_afiliado(url_puro)
            if link_af:
                # Se a loja for reconhecida, esse passa a ser o link do botao
                if not link_principal_afiliado or _detectar_loja(url_puro) != "other":
                    link_principal_afiliado = link_af
                
                # Remove a URL pura do corpo da mensagem
                copy_gerada = copy_gerada.replace(url_puro, "")
        
        # Limpeza de labels residuais e espacos extras
        copy_gerada = re.sub(r'(?i)(?:link|compre aqui|oferta|🛒 pegar oferta|pegar oferta|🛒 compre aqui|clique aqui)[:\s]*$', '', copy_gerada, flags=re.MULTILINE)
        copy_gerada = re.sub(r'\n\s*\n', '\n\n', copy_gerada).strip()
        
        # Garante que o usuario saiba que deve clicar no botao
        if link_principal_afiliado and "botao abaixo" not in copy_gerada.lower() and len(copy_gerada) > 10:
            copy_gerada += "\n\n🔗 <b>Acesse a oferta clicando no botão abaixo! 👇</b>"

        # Se nao houver link de afiliado, usa o original para o botao
        link_temp = link_principal_afiliado or (urls[0] if urls else None)
        
        # Encurta para publicacao (mascara o link longo de afiliado)
        if link_temp:
            logger.info(f"[FORWARD] Encurtando link para o botao: {link_temp[:50]}...")
            link_final_botao = shorten_for_publication(link_temp)
        else:
            link_final_botao = "Sem link"

        # Mantem a deteccao de precos e cupons apenas para metadados/revisao
        precos = re.findall(r'R\$\s*[\d.,]+', texto_original)
        preco = precos[0] if precos else "Confira o preco"

        # No encaminhamento, NAO tentamos adivinhar cupom do meio do texto
        # Isso evita que palavras como "VALOR" ou "OFERTA" sejam confundidas com cupons
        cupom = item.get("cupom") 
        
        # Passo 3
        prog_texto = (
            "⚙️ <b>Processando suas promocoes...</b>\n\n"
            f"📦 Processando <b>{atual}/{total}</b>: {titulo_resumo}...\n"
        ...
        item_revisao = {
            ...
            "cupom": cupom,
            "aprovado": False,
            "preserve_fidelity": True, # FLAG CRITICA: Mantem o texto original
            "titulo": titulo_resumo, # Adicionado para compatibilidade com gerar_copy se necessario
            "nome": titulo_resumo,
            "imagem": midia.get("file_id") if isinstance(midia, dict) else midia,
            "created_at": datetime.now().isoformat()
        }

        # 1. Fila de revisao do usuario (para "Aprovar Todas" funcionar)
        ...
        if atual < total:
            prog_texto = (
                "⚙️ <b>Processando suas promocoes...</b>\n\n"
                f"✅ <b>{atual}/{total} concluida</b> — {titulo_resumo}\n"
    ...
    res_texto = (
        ...
        f"✅ {sucesso_qtd} promocoes identificadas\n"
    ...
    keyboard = [
        [InlineKeyboardButton("👀 Abrir Fila de Revisao (Mission Control)", callback_data="review_view:0")],
...
async def show_next_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not fila:
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text="🎉 <b>Revisao concluida!</b>\nTodas as promocoes foram processadas.",
            parse_mode="HTML"
        )
        from bot.handlers.start import start_command
        await asyncio.sleep(2)
    ...
    # Fallback: usa o tamanho atual da fila de revisao se por algum motivo nao foi salvo
    ...
    dica = "\n💡 <b>Dica:</b> Use o botao abaixo para adicionar ou editar o cupom!" if not cupom_val else ""
    
    msg_texto = (
        f"💎 <b>PREVIA — {atual} de {total}</b>\n\n"
        f"{item['copy']}\n"
    )
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("❌ Descartar", callback_data="frev_descartar"),
         InlineKeyboardButton("⏭️ Ver Proxima", callback_data="frev_proxima")],
        [InlineKeyboardButton("🏠 Voltar ao Menu", callback_data="menu_principal")]
    ]
...
async def frev_cupom(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(query.message.chat_id, "🎫 <b>MODO CUPOM</b>\n\nEnvie apenas o codigo do cupom abaixo (ou envie 'remover' para tirar):", parse_mode="HTML")
