...
async def encam_agendar_este_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Exibe op��es de tempo para agendar apenas O POST ATUAL."""
    query = update.callback_query
    await query.answer()

    texto = (
        "?? <b>Agendar ESTA Promo��o</b>\n\n"
        "Em quanto tempo voc� deseja que esta promo��o seja postada?\n"
...
async def encam_agendar_este_exec(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # O item sendo revisado � o FILA[0]
    item = fila.pop(0)
    context.user_data["fila_revisao"] = fila
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"? <b>Agendado!</b>\nEsta promo��o entrar� na fila de postagem (aprox. {minutos} min).",
        parse_mode="HTML"
    )
